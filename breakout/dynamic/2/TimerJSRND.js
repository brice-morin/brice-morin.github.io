...
TimerJSRND.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x8A, 0xBE), 0);
	});
}
TimerJSRND.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0x66, 0x12), 0);
...
TimerJSRND.prototype.receivetimer_startOntimer = function(time, id, var212, var237) {
	this._receive({_port:"timer", _msg:"timer_start", time:time, id:id, var212:var212, var237:var237});
}

TimerJSRND.prototype.receivetimer_cancelOntimer = function(var213, id, var236) {
	this._receive({_port:"timer", _msg:"timer_cancel", var213:var213, id:id, var236:var236});
}
