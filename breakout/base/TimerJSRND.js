...
TimerJSRND.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id), 0);
	});
...
TimerJSRND.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const), 0);
...
TimerJSRND.prototype.receivetimer_startOntimer = function(id, time) {
	this._receive({_port:"timer", _msg:"timer_start", id:id, time:time});
}

TimerJSRND.prototype.receivetimer_cancelOntimer = function(id) {
