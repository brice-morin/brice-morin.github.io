...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', 0x65, 0xC9, timer_start.id), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', 0x7C, 0x0C, id_const), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(val0, time, val25, id) {
	this._receive({_port:"timer", _msg:"timer_start", val0:val0, time:time, val25:val25, id:id});
}
