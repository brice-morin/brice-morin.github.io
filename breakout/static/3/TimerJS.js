...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x72, 0x49), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xCA, 0x18), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(val222, val198, time, id) {
	this._receive({_port:"timer", _msg:"timer_start", val222:val222, val198:val198, time:time, id:id});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(val221, val199, id) {
	this._receive({_port:"timer", _msg:"timer_cancel", val221:val221, val199:val199, id:id});
}
