...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x1F, 0xA0), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xFA, 0x46), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(id, time, val156, val132) {
	this._receive({_port:"timer", _msg:"timer_start", id:id, time:time, val156:val156, val132:val132});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(val133, id, val155) {
	this._receive({_port:"timer", _msg:"timer_cancel", val133:val133, id:id, val155:val155});
}
