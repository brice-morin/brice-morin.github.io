...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', 0xCF, 0xD5, timer_start.id), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', 0x66, 0x18, id_const), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(time, val288, id, val264) {
	this._receive({_port:"timer", _msg:"timer_start", time:time, val288:val288, id:id, val264:val264});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(val289, val265, id) {
	this._receive({_port:"timer", _msg:"timer_cancel", val289:val289, val265:val265, id:id});
}
