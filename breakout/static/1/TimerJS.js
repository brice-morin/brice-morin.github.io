...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', 0x11, timer_start.id, 0xF2), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', 0x10, id_const, 0x58), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(id, time, val90, val66) {
	this._receive({_port:"timer", _msg:"timer_start", id:id, time:time, val90:val90, val66:val66});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(val91, val67, id) {
	this._receive({_port:"timer", _msg:"timer_cancel", val91:val91, val67:val67, id:id});
}
