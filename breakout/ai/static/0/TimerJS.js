...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', 0x90, 0x65, timer_start.id), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', 0x0A, 0x7C, id_const), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(id, val25, time, val0) {
	this._receive({_port:"timer", _msg:"timer_start", id:id, val25:val25, time:time, val0:val0});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(id, val23, val1) {
	this._receive({_port:"timer", _msg:"timer_cancel", id:id, val23:val23, val1:val1});
}
