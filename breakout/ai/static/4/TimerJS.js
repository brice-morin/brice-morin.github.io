...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0xDF, 0x86), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xAF, 0x8F), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(val288, id, time, val264) {
	this._receive({_port:"timer", _msg:"timer_start", val288:val288, id:id, time:time, val264:val264});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(val287, val265, id) {
	this._receive({_port:"timer", _msg:"timer_cancel", val287:val287, val265:val265, id:id});
}
