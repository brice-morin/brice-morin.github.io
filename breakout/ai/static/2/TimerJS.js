...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x0C, 0x18), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xCA, 0xCB), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(val132, id, val157, time) {
	this._receive({_port:"timer", _msg:"timer_start", val132:val132, id:id, val157:val157, time:time});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(id, val133, val156) {
	this._receive({_port:"timer", _msg:"timer_cancel", id:id, val133:val133, val156:val156});
}
