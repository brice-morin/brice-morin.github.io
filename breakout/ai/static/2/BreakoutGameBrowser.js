...
BreakoutGameBrowser.prototype.build = function(session) {
	...
	let BreakoutGame_SC_INIT = new StateJS.State('INIT', this._statemachine).entry(() => {
		this.bus.emit('display?createval142ysize', 0x57, 0x16, this.BreakoutGame_YDISPSIZE_var);
		this.bus.emit('display?createxsize', 0xAD, this.BreakoutGame_XDISPSIZE_var);
	});
	let BreakoutGame_SC_LAUNCH = new StateJS.State('LAUNCH', this._statemachine).entry(() => {
		setTimeout(() => this.bus.emit('clock?timer_start', 0xDD, 0, 0x52, 33), 0);
		this.BreakoutGame_SC_LAUNCH_countdown_var = 30 * 3;
		this.drawScore();
		this.drawLives();
		this.bus.emit('display?update_', 0x01);
		this.bus.emit('display?updateval144', 0x89, 0xFD);
	});
	let BreakoutGame_SC_PLAY = new StateJS.State('PLAY', this._statemachine).entry(() => {
		const period_const = this.BreakoutGame_period_var;
		setTimeout(() => this.bus.emit('clock?timer_start', 0xFE, 0, 0x2A, period_const), 0);
	});
	let BreakoutGame_SC_LOSTBALL = new StateJS.State('LOSTBALL', this._statemachine).entry(() => {
		setTimeout(() => this.bus.emit('clock?timer_start', 0x35, 0, 0xE8, 500), 0);
		this.BreakoutGame_lives_var = this.BreakoutGame_lives_var - 1;
		this.bus.emit('lives=', this.BreakoutGame_lives_var);
		this.eraseBall();
		this.erasePad();
		this.drawLives();
		this.bus.emit('display?update_', 0x7A);
		this.bus.emit('display?updateval144', 0x70, 0xCD);
		this.log(true);
	});
	let BreakoutGame_SC_NEXTLEVEL = new StateJS.State('NEXTLEVEL', this._statemachine).entry(() => {
		setTimeout(() => this.bus.emit('clock?timer_start', 0x00, 0, 0xAB, 1000), 0);
		...
		this.bus.emit('display?updateval144', 0x04, 0x35);
	});
	let BreakoutGame_SC_GAMEOVER = new StateJS.State('GAMEOVER', this._statemachine).entry(() => {
		this.eraseBall();
		this.erasePad();
		this.bus.emit('display?updateval144', 0xE0, 0xBD);
		this.bus.emit('display?update_', 0x2B);
		setTimeout(() => this.bus.emit('clock?timer_start', 0xD5, 0, 0x1A, 500), 0);
	});
	let BreakoutGame_SC_GAMEREALLYOVER = new StateJS.State('GAMEREALLYOVER', this._statemachine).entry(() => {
		this.bus.emit('display?setColorval136', 0xFE, 0xC1);
		this.bus.emit('display?setColorgbr', 255, 255, 0x24, 255);
		this.bus.emit('display?fillRectxyval139', 8, 0x34, 30, 0x3E);
		this.bus.emit('display?fillRectheightwidth', 0x9A, 142, 76);
		this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
		, this.BreakoutGame_fgcolor_var[2]
		, 0xF7, this.BreakoutGame_fgcolor_var[1]
		);
		this.bus.emit('display?setColorval136', 0xA2, 0x5A);
		this.bus.emit('display?fillRectxyval139', 9, 0x59, 31, 0xF1);
		this.bus.emit('display?fillRectheightwidth', 0xEC, 140, 50);
		this.bus.emit('display?setBGColorrval137', this.BreakoutGame_fgcolor_var[0]
		, 0x91, 0x3A);
		this.bus.emit('display?setBGColorgb', 0x7F, this.BreakoutGame_fgcolor_var[1]
		, this.BreakoutGame_fgcolor_var[2]
		);
		this.bus.emit('display?setColorgbr', 158, 130, 0xE5, 209);
		this.bus.emit('display?setColorval136', 0x4B, 0x14);
		this.bus.emit('display?drawIntegerdigitsxscale', 6, 5, 0xAC, 23);
		this.bus.emit('display?drawIntegeryvval140', 0xDC, this.BreakoutGame_score_var, 40, 0x35);
		this.bus.emit('display?drawThingMLyval141x', 0x52, 26, 0x5A, 87);
		this.bus.emit('display?drawThingML_', 0x9D);
		this.bus.emit('display?updateval144', 0x65, 0x97);
		this.bus.emit('display?update_', 0xF9);
		this.log(true);
		this.quit();
	...
	BreakoutGame_SC_INIT.to(BreakoutGame_SC_LAUNCH).when((displayReady_) => {
		...
		this.bus.emit('display?clear_', 0x8F);
		this.bus.emit('display?clearval135', 0x03, 0x0D);
		this.initColors();
		this.bus.emit('display?setColorval136', 0x58, 0xAA);
		this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
		, this.BreakoutGame_bgcolor_var[2]
		, 0x04, this.BreakoutGame_bgcolor_var[1]
		);
		this.bus.emit('display?fillRectxyval139', 0, 0xC7, 0, 0x3B);
		this.bus.emit('display?fillRectheightwidth', 0xB0, this.BreakoutGame_XDISPSIZE_var, this.BreakoutGame_YDISPSIZE_var);
		this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
		, this.BreakoutGame_fgcolor_var[2]
		, 0x68, this.BreakoutGame_fgcolor_var[1]
		);
		this.bus.emit('display?setColorval136', 0xBE, 0xD8);
		this.bus.emit('display?fillRectxyval139', 0, 0xC1, 0, 0xC5);
		this.bus.emit('display?fillRectheightwidth', 0xBB, this.BreakoutGame_XDISPSIZE_var, 14);
		this.drawWalls();
		this.createBricks();
		this.drawLevel();
	...
	BreakoutGame_SC_INIT.to(BreakoutGame_SC_LAUNCH).when((displayReadyval145) => {
		...
		this.bus.emit('display?clearval135', 0xAC, 0x0D);
		this.initColors();
		this.bus.emit('display?setColorval136', 0x44, 0xAA);
		this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
		, this.BreakoutGame_bgcolor_var[2]
		, 0xA0, this.BreakoutGame_bgcolor_var[1]
		);
		this.bus.emit('display?fillRectheightwidth', 0x66, this.BreakoutGame_XDISPSIZE_var, this.BreakoutGame_YDISPSIZE_var);
		this.bus.emit('display?fillRectxyval139', 0, 0xC7, 0, 0xEB);
		this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
		, this.BreakoutGame_fgcolor_var[2]
		, 0x7F, this.BreakoutGame_fgcolor_var[1]
		);
		this.bus.emit('display?setColorval136', 0x14, 0xD8);
		this.bus.emit('display?fillRectheightwidth', 0x63, this.BreakoutGame_XDISPSIZE_var, 14);
		this.bus.emit('display?fillRectxyval139', 0, 0xC1, 0, 0xFF);
		this.drawWalls();
		this.createBricks();
		this.drawLevel();
		this.BreakoutGame_SC_INIT_received_display_displayReadyval145_var = false;
	...
	this._statemachine.to(null).when((positionval148y) => {
		return positionval148y._port === 'controller' && positionval148y._msg === 'positionval148y';
	}).effect((positionval148y) => {
		this.BreakoutGame_SC_received_controller_positionval148y_var = true;
		this.BreakoutGame_SC_controller_position_val148_var = positionval148y.val148;
		this.BreakoutGame_SC_controller_position_y_var = positionval148y.y;
		...
		this.BreakoutGame_SC_received_controller_positionval148y_var = false;
	...
	this._statemachine.to(null).when((positionx) => {
		...
		if(this.BreakoutGame_SC_received_controller_positionval148y_var) {
		let center_var = (this.BreakoutGame_RIGHT_var - this.BreakoutGame_LEFT_var - this.BreakoutGame_padlen_var);
		center_var = this.BreakoutGame_SC_controller_position_x_var * center_var;
		center_var = Math.trunc(center_var / 200);
		this.BreakoutGame_padx_var = (this.BreakoutGame_LEFT_var + center_var + Math.trunc((this.BreakoutGame_RIGHT_var - this.BreakoutGame_LEFT_var) / 2));
	...
	BreakoutGame_SC_LAUNCH.to(BreakoutGame_SC_PLAY).when((timer_timeout) => {
		...
		this.resetBall();
		this.bus.emit('display?update_', 0x47);
		this.bus.emit('display?updateval144', 0xDE, 0xD4);
	});
	BreakoutGame_SC_LAUNCH.to(null).when((timer_timeout) => {
		return timer_timeout._port === 'clock' && timer_timeout._msg === 'timer_timeout' && (timer_timeout.id === 0 && this.BreakoutGame_SC_LAUNCH_countdown_var > 0);
	}).effect((timer_timeout) => {
		setTimeout(() => this.bus.emit('clock?timer_start', 0xB2, 0, 0xAB, 33), 0);
		this.drawPad();
		...
		this.bus.emit('display?update_', 0x58);
		this.bus.emit('display?updateval144', 0xC0, 0x26);
	});
	...
	BreakoutGame_SC_PLAY.to(null).when((timer_timeout) => {
		...
		this.bus.emit('sound?tonefreqval154time', 0x9D, 0xA2, this.BreakoutGame_tone2_var, this.BreakoutGame_tone_duration_var);
		...
		this.bus.emit('sound?tone_', 0x7F);
		this.bus.emit('sound?tonefreqval154time', 0x2F, 0x29, this.BreakoutGame_tone2_var, this.BreakoutGame_tone_duration_var);
		this.BreakoutGame_dx_var =  -this.BreakoutGame_dx_var;
		this.bus.emit('dx=', this.BreakoutGame_dx_var);
		...
		this.bus.emit('sound?tone_', 0xB9);
		this.bus.emit('sound?tonefreqval154time', 0xC5, 0x2F, this.BreakoutGame_tone2_var, this.BreakoutGame_tone_duration_var);
		...
		setTimeout(() => this.bus.emit('game?lostBallval152', 0x5C, 0x16), 0);
		setTimeout(() => this.bus.emit('game?lostBall_', 0xB9), 0);
		setTimeout(() => this.bus.emit('req_game?lostBallval152', 0x76, 0xE8), 0);
		setTimeout(() => this.bus.emit('req_game?lostBall_', 0x42), 0);
		...
		this.bus.emit('sound?tonefreqval154time', 0x53, 0x84, this.BreakoutGame_tone2_var, this.BreakoutGame_tone_duration_var);
		this.BreakoutGame_dy_var =  -this.BreakoutGame_dy_var;
		this.bus.emit('dy=', this.BreakoutGame_dy_var);
		...
		if(collision_var) {
		this.bus.emit('sound?tone_', 0x62);
		this.bus.emit('sound?tonefreqval154time', 0x05, 0x13, this.BreakoutGame_tone1_var, this.BreakoutGame_tone_duration_var);
		this.BreakoutGame_dy_var =  -this.BreakoutGame_dy_var;
		this.bus.emit('dy=', this.BreakoutGame_dy_var);
		this.incrementScore(10);
		if(this.bricksLeft() === 0) {
		setTimeout(() => this.bus.emit('game?nextLevelval153', 0x4B, 0xFD), 0);
		setTimeout(() => this.bus.emit('game?nextLevel_', 0xA4), 0);
		setTimeout(() => this.bus.emit('req_game?nextLevelval153', 0x4C, 0xB7), 0);
		setTimeout(() => this.bus.emit('req_game?nextLevel_', 0x9F), 0);
		...
		this.drawBall();
		this.drawPad();
		setTimeout(() => this.bus.emit('ia?updateIA_', 0x45), 0);
		...
		setTimeout(() => this.bus.emit('ia?updateIAballxballypadyval149padx', 0x9D, by_const, bx_const, pady_const, padx_const, 0x8F), 0);
		this.bus.emit('display?update_', 0x46);
		this.bus.emit('display?updateval144', 0x9B, 0xEF);
		this.log(false);
		const period_const = this.BreakoutGame_period_var;
		setTimeout(() => this.bus.emit('clock?timer_start', 0x4A, 0, 0x95, period_const), 0);
	});
	...
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_NEXTLEVEL).when((nextLevel_) => {
		return nextLevel_._port === 'pro_game' && nextLevel_._msg === 'nextLevel_' && (this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var);
	}).effect((nextLevel_) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0x37, 0xEA), 0);
		this.BreakoutGame_SC_PLAY_received_pro_game_nextLevel__var = false;
		this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var = false;
	});
	...
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_LOSTBALL).when((lostBallval152) => {
		return lostBallval152._port === 'pro_game' && lostBallval152._msg === 'lostBallval152' && (this.BreakoutGame_SC_PLAY_received_pro_game_lostBall__var);
	}).effect((lostBallval152) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0xAB, 0x66), 0);
		this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var = false;
		this.BreakoutGame_SC_PLAY_received_pro_game_lostBall__var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((lostBallval152) => {
		return lostBallval152._port === 'pro_game' && lostBallval152._msg === 'lostBallval152' && (!(this.BreakoutGame_SC_PLAY_received_pro_game_lostBall__var));
	}).effect((lostBallval152) => {
		this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var = true;
	});
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_NEXTLEVEL).when((nextLevelval153) => {
		return nextLevelval153._port === 'pro_game' && nextLevelval153._msg === 'nextLevelval153' && (this.BreakoutGame_SC_PLAY_received_pro_game_nextLevel__var);
	}).effect((nextLevelval153) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0x37, 0xE3), 0);
		this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var = false;
		this.BreakoutGame_SC_PLAY_received_pro_game_nextLevel__var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((nextLevelval153) => {
		return nextLevelval153._port === 'pro_game' && nextLevelval153._msg === 'nextLevelval153' && (!(this.BreakoutGame_SC_PLAY_received_pro_game_nextLevel__var));
	}).effect((nextLevelval153) => {
		this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var = true;
	});
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_LOSTBALL).when((lostBall_) => {
		return lostBall_._port === 'pro_game' && lostBall_._msg === 'lostBall_' && (this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var);
	}).effect((lostBall_) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0xAB, 0x38), 0);
		this.BreakoutGame_SC_PLAY_received_pro_game_lostBall__var = false;
		this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var = false;
	});
	...
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_NEXTLEVEL).when((nextLevel_) => {
		return nextLevel_._port === 'game' && nextLevel_._msg === 'nextLevel_' && (this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var);
	}).effect((nextLevel_) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0xFC, 0x1E), 0);
		this.BreakoutGame_SC_PLAY_received_game_nextLevel__var = false;
		this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((nextLevel_) => {
		return nextLevel_._port === 'game' && nextLevel_._msg === 'nextLevel_' && (!(this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var));
	}).effect((nextLevel_) => {
	...
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_LOSTBALL).when((lostBallval152) => {
		return lostBallval152._port === 'game' && lostBallval152._msg === 'lostBallval152' && (this.BreakoutGame_SC_PLAY_received_game_lostBall__var);
	}).effect((lostBallval152) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0x1E, 0x74), 0);
		this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var = false;
		this.BreakoutGame_SC_PLAY_received_game_lostBall__var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((lostBallval152) => {
		return lostBallval152._port === 'game' && lostBallval152._msg === 'lostBallval152' && (!(this.BreakoutGame_SC_PLAY_received_game_lostBall__var));
	}).effect((lostBallval152) => {
		this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var = true;
	});
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_NEXTLEVEL).when((nextLevelval153) => {
		return nextLevelval153._port === 'game' && nextLevelval153._msg === 'nextLevelval153' && (this.BreakoutGame_SC_PLAY_received_game_nextLevel__var);
	}).effect((nextLevelval153) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0xFC, 0x12), 0);
		this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var = false;
		this.BreakoutGame_SC_PLAY_received_game_nextLevel__var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((nextLevelval153) => {
		...
		this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var = true;
	});
	BreakoutGame_SC_PLAY.to(BreakoutGame_SC_LOSTBALL).when((lostBall_) => {
		return lostBall_._port === 'game' && lostBall_._msg === 'lostBall_' && (this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var);
	}).effect((lostBall_) => {
		setTimeout(() => this.bus.emit('clock?timer_cancel', 0, 0x1E, 0xAD), 0);
		this.BreakoutGame_SC_PLAY_received_game_lostBall__var = false;
		this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var = false;
	});
	BreakoutGame_SC_PLAY.to(null).when((lostBall_) => {
...
BreakoutGameBrowser.prototype.initColors = function() {
	...
	this.bus.emit('fgcolor=', this.BreakoutGame_fgcolor_var);
	this.bus.emit('display?setBGColorrval137', this.BreakoutGame_bgcolor_var[0]
	, 0x20, 0x0D);
	this.bus.emit('display?setBGColorgb', 0xD2, this.BreakoutGame_bgcolor_var[1]
	, this.BreakoutGame_bgcolor_var[2]
	);
	this.bus.emit('display?setColorval136', 0x23, 0xBA);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0x24, this.BreakoutGame_fgcolor_var[1]
	);
}
...
BreakoutGameBrowser.prototype.eraseBall = function() {
	...
	this.bus.emit('display?setColorval136', 0x48, 0x81);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
	, this.BreakoutGame_bgcolor_var[2]
	, 0x3F, this.BreakoutGame_bgcolor_var[1]
	);
	this.bus.emit('display?fillRectxyval139', this.BreakoutGame_prevBX_var, 0x7D, this.BreakoutGame_prevBY_var, 0x46);
	this.bus.emit('display?fillRectheightwidth', 0xA0, bs_var, bs_var);
...
BreakoutGameBrowser.prototype.drawBall = function() {
	...
	this.bus.emit('prevBY=', this.BreakoutGame_prevBY_var);
	this.bus.emit('display?setColorgbr', 183, 111, 0xDB, 199);
	this.bus.emit('display?setColorval136', 0xEF, 0xAB);
	this.bus.emit('display?fillRectheightwidth', 0xC7, bs_var, bs_var);
	this.bus.emit('display?fillRectxyval139', this.BreakoutGame_prevBX_var, 0xFA, this.BreakoutGame_prevBY_var, 0x61);
}
...
BreakoutGameBrowser.prototype.erasePad = function() {
	...
	this.bus.emit('display?setColorval136', 0xEE, 0x1F);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
	, this.BreakoutGame_bgcolor_var[2]
	, 0xF0, this.BreakoutGame_bgcolor_var[1]
	);
	this.bus.emit('display?fillRectheightwidth', 0xB0, ps_var, 4);
	this.bus.emit('display?fillRectxyval139', this.BreakoutGame_prevPX_var, 0x95, this.BreakoutGame_prevPY_var, 0x5B);
...
BreakoutGameBrowser.prototype.drawPad = function() {
	...
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0x95, this.BreakoutGame_fgcolor_var[1]
	);
	this.bus.emit('display?setColorval136', 0x19, 0xF5);
	this.bus.emit('display?fillRectheightwidth', 0xB7, ps_var, 4);
	this.bus.emit('display?fillRectxyval139', this.BreakoutGame_prevPX_var, 0xF4, this.BreakoutGame_prevPY_var, 0x11);
}

BreakoutGameBrowser.prototype.drawCountDown = function(BreakoutGame_drawCountDown_c_var) {
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0xAC, this.BreakoutGame_fgcolor_var[1]
	);
	this.bus.emit('display?setColorval136', 0x03, 0x80);
	if(BreakoutGame_drawCountDown_c_var > 0) {
	this.bus.emit('display?setColorval136', 0xA5, 0x67);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0x33, this.BreakoutGame_fgcolor_var[1]
	);
	this.bus.emit('display?setBGColorgb', 0x9D, this.BreakoutGame_bgcolor_var[1]
	, this.BreakoutGame_bgcolor_var[2]
	);
	this.bus.emit('display?setBGColorrval137', this.BreakoutGame_bgcolor_var[0]
	, 0x1B, 0x9D);
	this.bus.emit('display?drawIntegerdigitsxscale', 4, 1, 0x25, 80 - 6);
	this.bus.emit('display?drawIntegeryvval140', 0xD0, BreakoutGame_drawCountDown_c_var, 90, 0x1C);
	...
	this.bus.emit('display?setColorval136', 0xC2, 0x7A);
	this.bus.emit('display?fillRectheightwidth', 0xC9, 12, 20);
	this.bus.emit('display?fillRectxyval139', 80 - 6, 0xF4, 90, 0x49);
...
BreakoutGameBrowser.prototype.drawWalls = function() {
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0x17, this.BreakoutGame_fgcolor_var[1]
	);
	this.bus.emit('display?setColorval136', 0x12, 0xFB);
	...
	this.bus.emit('display?fillRectheightwidth', 0x1B, xcenter_var + 1, 1);
	this.bus.emit('display?fillRectxyval139', left_var - 1, 0xF8, top_var - 1, 0x79);
	this.bus.emit('display?fillRectxyval139', left_var - 1, 0x65, bottom_var, 0xCA);
	this.bus.emit('display?fillRectheightwidth', 0x87, xcenter_var + 1, 1);
	this.bus.emit('display?fillRectheightwidth', 0x68, 1, ycenter_var);
	this.bus.emit('display?fillRectxyval139', left_var - 1, 0x79, top_var, 0x2C);
	this.bus.emit('display?fillRectheightwidth', 0x8E, 1, ycenter_var);
	this.bus.emit('display?fillRectxyval139', right_var, 0x36, top_var, 0xFF);
}
...
BreakoutGameBrowser.prototype.drawBrick = function(BreakoutGame_drawBrick_x_var, BreakoutGame_drawBrick_y_var) {
	...
	this.bus.emit('display?setColorgbr', 155, 89, 0x70, 103);
	this.bus.emit('display?setColorval136', 0x57, 0x49);
	this.bus.emit('display?fillRectheightwidth', 0x56, w_var, h_var);
	this.bus.emit('display?fillRectxyval139', bx_var, 0x8F, by_var, 0xDE);
	this.bus.emit('display?setColorgbr', 100, 43, 0x28, 56);
	this.bus.emit('display?setColorval136', 0xB4, 0x55);
	this.bus.emit('display?drawRectwidthval138heightx', 0x9D, w_var, bx_var, h_var, 0xA5);
	this.bus.emit('display?drawRecty', by_var, 0xC8);
}
...
BreakoutGameBrowser.prototype.removeBrick = function(BreakoutGame_removeBrick_x_var, BreakoutGame_removeBrick_y_var) {
	...
	this.bus.emit('display?setColorval136', 0x54, 0xB9);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
	, this.BreakoutGame_bgcolor_var[2]
	, 0x91, this.BreakoutGame_bgcolor_var[1]
	);
	this.bus.emit('display?fillRectxyval139', bx_var, 0xBC, by_var, 0x6A);
	this.bus.emit('display?fillRectheightwidth', 0x87, (Math.trunc((Math.trunc((this.BreakoutGame_RIGHT_var - this.BreakoutGame_LEFT_var) / 8)) / this.BreakoutGame_SCALE_var) - 2), this.BreakoutGame_BRICK_HEIGHT_var - 2);
...
BreakoutGameBrowser.prototype.drawLevel = function() {
	this.bus.emit('display?setColorval136', 0xB4, 0xDE);
	this.bus.emit('display?setColorgbr', 158, 130, 0xD3, 209);
	this.bus.emit('display?setBGColorgb', 0x46, this.BreakoutGame_fgcolor_var[1]
	, this.BreakoutGame_fgcolor_var[2]
	);
	this.bus.emit('display?setBGColorrval137', this.BreakoutGame_fgcolor_var[0]
	, 0x1E, 0x21);
	this.bus.emit('display?setColorval136', 0x9D, 0x82);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_bgcolor_var[0]
	, this.BreakoutGame_bgcolor_var[2]
	, 0xA9, this.BreakoutGame_bgcolor_var[1]
	);
	this.bus.emit('display?drawIntegerdigitsxscale', 2, 2, 0x07, 6);
	this.bus.emit('display?drawIntegeryvval140', 0x57, this.BreakoutGame_level_var, 2, 0xD5);
}
...
BreakoutGameBrowser.prototype.drawScore = function() {
	this.bus.emit('display?setColorgbr', 158, 130, 0xAB, 209);
	this.bus.emit('display?setColorval136', 0xB4, 0x50);
	this.bus.emit('display?setBGColorrval137', this.BreakoutGame_fgcolor_var[0]
	, 0x0F, 0xDF);
	this.bus.emit('display?setBGColorgb', 0x66, this.BreakoutGame_fgcolor_var[1]
	, this.BreakoutGame_fgcolor_var[2]
	);
	this.bus.emit('display?drawIntegeryvval140', 0xC0, this.BreakoutGame_score_var, 2, 0x03);
	this.bus.emit('display?drawIntegerdigitsxscale', 2, 5, 0x22, 58);
}

BreakoutGameBrowser.prototype.drawLives = function() {
	this.bus.emit('display?setColorval136', 0x73, 0xE9);
	this.bus.emit('display?setColorgbr', this.BreakoutGame_fgcolor_var[0]
	, this.BreakoutGame_fgcolor_var[2]
	, 0xF2, this.BreakoutGame_fgcolor_var[1]
	);
	this.bus.emit('display?fillRectxyval139', 124, 0xC0, 4, 0xE6);
	this.bus.emit('display?fillRectheightwidth', 0xA5, 24 + 6, 6);
	this.bus.emit('display?setColorval136', 0x75, 0x67);
	this.bus.emit('display?setColorgbr', 183, 111, 0x45, 199);
	let i_var = 0;
	while(i_var < this.BreakoutGame_lives_var) {
	this.bus.emit('display?fillRectxyval139', 124 + (2 - i_var) * 12, 0x80, 4, 0xB1);
	this.bus.emit('display?fillRectheightwidth', 0x85, 6, 6);
	i_var = i_var + 1;
...
BreakoutGameBrowser.prototype.receivetimer_timeoutOnclock = function(id, val134, val155) {
	this._receive({_port:"clock", _msg:"timer_timeout", id:id, val134:val134, val155:val155});
}

BreakoutGameBrowser.prototype.receivedisplayReady_Ondisplay = function(val176) {
	this._receive({_port:"display", _msg:"displayReady_", val176:val176});
}

BreakoutGameBrowser.prototype.receivedisplayReadyval145Ondisplay = function(val145, val177) {
	this._receive({_port:"display", _msg:"displayReadyval145", val145:val145, val177:val177});
}

BreakoutGameBrowser.prototype.receivedisplayError_Ondisplay = function(val166) {
	this._receive({_port:"display", _msg:"displayError_", val166:val166});
}

BreakoutGameBrowser.prototype.receivedisplayErrorval146Ondisplay = function(val167, val146) {
	this._receive({_port:"display", _msg:"displayErrorval146", val167:val167, val146:val146});
}

BreakoutGameBrowser.prototype.receivepositionxOncontroller = function(val184, x) {
	this._receive({_port:"controller", _msg:"positionx", val184:val184, x:x});
}

BreakoutGameBrowser.prototype.receivepositionval148yOncontroller = function(y, val148, val185) {
	this._receive({_port:"controller", _msg:"positionval148y", y:y, val148:val148, val185:val185});
}

BreakoutGameBrowser.prototype.receivelostBall_Ongame = function(val194) {
	this._receive({_port:"game", _msg:"lostBall_", val194:val194});
}

BreakoutGameBrowser.prototype.receivelostBallval152Ongame = function(val152, val195) {
	this._receive({_port:"game", _msg:"lostBallval152", val152:val152, val195:val195});
}

BreakoutGameBrowser.prototype.receivenextLevel_Ongame = function(val192) {
	this._receive({_port:"game", _msg:"nextLevel_", val192:val192});
}

BreakoutGameBrowser.prototype.receivenextLevelval153Ongame = function(val153, val193) {
	this._receive({_port:"game", _msg:"nextLevelval153", val153:val153, val193:val193});
}

BreakoutGameBrowser.prototype.receivelostBall_Onpro_game = function(val194) {
	this._receive({_port:"pro_game", _msg:"lostBall_", val194:val194});
}

BreakoutGameBrowser.prototype.receivelostBallval152Onpro_game = function(val152, val195) {
	this._receive({_port:"pro_game", _msg:"lostBallval152", val152:val152, val195:val195});
}

BreakoutGameBrowser.prototype.receivenextLevel_Onpro_game = function(val192) {
	this._receive({_port:"pro_game", _msg:"nextLevel_", val192:val192});
}

BreakoutGameBrowser.prototype.receivenextLevelval153Onpro_game = function(val153, val193) {
	this._receive({_port:"pro_game", _msg:"nextLevelval153", val153:val153, val193:val193});
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var = function(BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var) {
	this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var = BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_received_controller_positionval148y_var = function(BreakoutGame_SC_received_controller_positionval148y_var) {
	this.BreakoutGame_SC_received_controller_positionval148y_var = BreakoutGame_SC_received_controller_positionval148y_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_game_nextLevel_val153_var = function(BreakoutGame_SC_PLAY_game_nextLevel_val153_var) {
	this.BreakoutGame_SC_PLAY_game_nextLevel_val153_var = BreakoutGame_SC_PLAY_game_nextLevel_val153_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var = function(BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var) {
	this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var = BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_received_game_nextLevelval153_var = function(BreakoutGame_SC_PLAY_received_game_nextLevelval153_var) {
	this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var = BreakoutGame_SC_PLAY_received_game_nextLevelval153_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_pro_game_nextLevel_val153_var = function(BreakoutGame_SC_PLAY_pro_game_nextLevel_val153_var) {
	this.BreakoutGame_SC_PLAY_pro_game_nextLevel_val153_var = BreakoutGame_SC_PLAY_pro_game_nextLevel_val153_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_pro_game_lostBall_val152_var = function(BreakoutGame_SC_PLAY_pro_game_lostBall_val152_var) {
	this.BreakoutGame_SC_PLAY_pro_game_lostBall_val152_var = BreakoutGame_SC_PLAY_pro_game_lostBall_val152_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_received_game_lostBallval152_var = function(BreakoutGame_SC_PLAY_received_game_lostBallval152_var) {
	this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var = BreakoutGame_SC_PLAY_received_game_lostBallval152_var;
}
...
BreakoutGameBrowser.prototype.initBreakoutGame_SC_PLAY_game_lostBall_val152_var = function(BreakoutGame_SC_PLAY_game_lostBall_val152_var) {
	this.BreakoutGame_SC_PLAY_game_lostBall_val152_var = BreakoutGame_SC_PLAY_game_lostBall_val152_var;
}
...
BreakoutGameBrowser.prototype.toString = function() {
	...
	result += '\n\treceived_pro_game_nextLevelval153 = ' + this.BreakoutGame_SC_PLAY_received_pro_game_nextLevelval153_var;
	...
	result += '\n\treceived_controller_positionval148y = ' + this.BreakoutGame_SC_received_controller_positionval148y_var;
	...
	result += '\n\tgame_nextLevel_val153 = ' + this.BreakoutGame_SC_PLAY_game_nextLevel_val153_var;
	result += '\n\tBRICK_HEIGHT = ' + this.BreakoutGame_BRICK_HEIGHT_var;
	result += '\n\tlevel = ' + this.BreakoutGame_level_var;
	result += '\n\tprevPY = ' + this.BreakoutGame_prevPY_var;
	result += '\n\treceived_pro_game_lostBallval152 = ' + this.BreakoutGame_SC_PLAY_received_pro_game_lostBallval152_var;
	...
	result += '\n\treceived_game_nextLevelval153 = ' + this.BreakoutGame_SC_PLAY_received_game_nextLevelval153_var;
	result += '\n\tpadx = ' + this.BreakoutGame_padx_var;
	result += '\n\tscore = ' + this.BreakoutGame_score_var;
	result += '\n\tXMAX = ' + this.BreakoutGame_XMAX_var;
	result += '\n\tpro_game_nextLevel_val153 = ' + this.BreakoutGame_SC_PLAY_pro_game_nextLevel_val153_var;
	result += '\n\tYMAX = ' + this.BreakoutGame_YMAX_var;
	result += '\n\tcountdown = ' + this.BreakoutGame_SC_LAUNCH_countdown_var;
	result += '\n\tpady = ' + this.BreakoutGame_pady_var;
	result += '\n\tpro_game_lostBall_val152 = ' + this.BreakoutGame_SC_PLAY_pro_game_lostBall_val152_var;
	result += '\n\tdx = ' + this.BreakoutGame_dx_var;
	result += '\n\treceived_game_lostBallval152 = ' + this.BreakoutGame_SC_PLAY_received_game_lostBallval152_var;
	result += '\n\tbx = ' + this.BreakoutGame_bx_var;
	result += '\n\tBOTTOM = ' + this.BreakoutGame_BOTTOM_var;
	result += '\n\tgame_lostBall_val152 = ' + this.BreakoutGame_SC_PLAY_game_lostBall_val152_var;
