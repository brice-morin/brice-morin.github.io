...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x00, 0x48), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xE8, 0xD0), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(val223, id, val198, time) {
	this._receive({_port:"timer", _msg:"timer_start", val223:val223, id:id, val198:val198, time:time});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(id, val222, val199) {
	this._receive({_port:"timer", _msg:"timer_cancel", id:id, val222:val222, val199:val199});
}
