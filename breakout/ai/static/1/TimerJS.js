...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0x71, 0xB3), 0);
	});
...
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0xA9, 0x1B), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(time, id, val89, val66) {
	this._receive({_port:"timer", _msg:"timer_start", time:time, id:id, val89:val89, val66:val66});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(id, val67, val90) {
	this._receive({_port:"timer", _msg:"timer_cancel", id:id, val67:val67, val90:val90});
}
