...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0xFC, 0xE8), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0x06, 0x75), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(var318, id, time, var341) {
	this._receive({_port:"timer", _msg:"timer_start", var318:var318, id:id, time:time, var341:var341});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(var342, var319, id) {
	this._receive({_port:"timer", _msg:"timer_cancel", var342:var342, var319:var319, id:id});
}
