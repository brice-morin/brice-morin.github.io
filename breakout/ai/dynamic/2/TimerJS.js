...
TimerJS.prototype.build = function(session) {
	...
	TimerJS_SoftTimer_default.to(null).when((timer_start) => {
		...
		setTimeout(() => this.bus.emit('timer?timer_timeout', timer_start.id, 0xE2, 0x0B), 0);
	});
}
TimerJS.prototype.startTimer = function(TimerJS_startTimer_id_var, TimerJS_startTimer_delay_var) {
	...
	setTimeout(() => this.bus.emit('timer?timer_timeout', id_const, 0x31, 0x99), 0);
...
TimerJS.prototype.receivetimer_startOntimer = function(id, var237, var212, time) {
	this._receive({_port:"timer", _msg:"timer_start", id:id, var237:var237, var212:var212, time:time});
}

TimerJS.prototype.receivetimer_cancelOntimer = function(var213, id, var236) {
